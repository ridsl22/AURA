...
namespace MetromTablet.Communication
{
	...
	[System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Naming", "CA1709:IdentifiersShouldBeCasedCorrectly",
		MessageId = "CRC", Justification = "Matching algorithm acronym.")]
	public class CRC32 : HashAlgorithm
	{
		...
		[System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Performance", "CA1810:InitializeReferenceTypeStaticFieldsInline", 
			Justification =	"Table values must be computed; not possible to remove the static constructor.")]
		static CRC32()
		{
			// Allocate table
			...
			for (uint n = 0; n < 256; n++)
			{
				...
				for (int k = 0; k < 8; k++)
				{
					...
					if (0 != (c & 1))
					{
						c = 0xEDB88320 ^ (c >> 1);//reversed
						//c = 0x04C11DB7 ^ (c >> 1);//normal
					}
					else
					{
						c = c >> 1;
